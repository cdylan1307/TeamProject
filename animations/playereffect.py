...
class Animation:
    ...
    def __init__(self, folder_path, frame_count, loop=True):
        ...
        if not self.frames:
            # Create placeholder if no frames loaded - use default player.png or create fallback
            try:
                # Try to load default player.png as fallback
                fallback_img = pygame.image.load("player.png").convert_alpha()
                self.frames.append(fallback_img)
                print(f"✓ Using player.png as fallback for {folder_path}")
            except:
                print(f"WARNING: No frames loaded for {folder_path}, using colored placeholder")
...
class Player:
    ...
    def draw(self, surface):
        ...
        # Removed health bar as requested
...
class Patroclus:
    ...
    def draw(self, surface):
        ...
        # Removed health/healing timer indicator as requested
