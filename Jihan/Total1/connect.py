...
pygame.display.set_caption("Pixel Menu + Level 1")
FPS = 60
...
# Only proceed if we have frames; otherwise avoid crashes
if len(attack_frames) == 0:
    # Dummy surface so code doesn't break; you can remove this
    attack_frames = [pygame.Surface((80, 80), pygame.SRCALPHA)]
# Load projectile image
if os.path.exists("arrow.png"):
    projectile_img = pygame.image.load("arrow.png").convert_alpha()
else:
    # Dummy projectile surface
    projectile_img = pygame.Surface((40, 10), pygame.SRCALPHA)
    pygame.draw.rect(projectile_img, (255, 255, 0), projectile_img.get_rect())
# Enemy position (Level 1)
enemy_x = 200
enemy_y = 500
# Animation control
current_frame = 0
...
projectiles = []
projectile_speed = 15
# Prevent spawning multiple projectiles at the same attack frame
...
while running:
    clock.tick(FPS)
    # Handle transition animation state
    if transition:
        ...
        if transition_progress >= WIDTH:
            ...
            # Optionally reset enemy / projectiles when entering level 1
            current_frame = 0
            frame_counter = 0
            projectiles = []
            projectile_spawned = False
    ...
    # UPDATE GAME LOGIC
    # ======================================================
    # Only run enemy logic when on the level 1 screen
    if scene == "start":
        # -------------------
        # Attack Animation
        # -------------------
        ...
        if frame_counter >= frame_delay:
            frame_counter = 0
            current_frame += 1
            # When it reaches frame 9 (ac9), spawn projectile once
            if current_frame == 9 and not projectile_spawned:
                projectile_spawned = True
                projectiles.append(
                    {"x": enemy_x + 80, "y": enemy_y + 30}
                )
            # Loop animation back to 0; reset projectile flag
    ...
    # START (LEVEL 1)
        ...
        # Optional: brief "GAME STARTED!" message
        if pygame.time.get_ticks() - start_time < 500:
            ...
            hint = pygame.font.SysFont("Courier New", 20).render(
    ...
    # CREDITS
        ...
        if credits_y + total_height < 0:
            credits_y = HEIGHT
    # ======================================================
    # TRANSITION EFFECT (MENU -> LEVEL 1)
    # ======================================================
    if transition:
        # Draw Level 1 background underneath
        if game_bg:
            SCREEN.blit(game_bg, (0, 0))
        else:
            SCREEN.fill((20, 120, 20))
        ...
        for i in range(NUM_SLICES):
            ...
            if background:
                ...
                # If no background image, create a simple colored slice
                slice_surface = pygame.Surface((slice_width, HEIGHT))
                slice_surface.fill(PLACEHOLDER_BG)
            # Alternate directions
            if i % 2 == 0:
                offset = -transition_progress
            else:
                offset = transition_progress
            SCREEN.blit(slice_surface, (x + offset, 0))
    pygame.display.flip()
pygame.quit()
sys.exit()
